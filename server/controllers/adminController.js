...
exports.createAdmin = [
  ...
  async (req, res) => {
    const { username, password, role, firstname, lastname, phone_number } =
      req.body;
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Username, password, role, firstname, lastname, and phone number are required.",
      });
    }
...
exports.getAllQuotations = [
  ...
  async (req, res) => {
    ...
    const queries = {
      ...
      `,
      // Add new queries for different moving types
      localMove: `
        SELECT *, 'local_move' AS type 
        FROM moving_service 
        WHERE JSON_CONTAINS(type_of_service, '"local_move"', '$')
      `,
      longDistanceMove: `
        SELECT *, 'long_distance_move' AS type 
        FROM moving_service 
        WHERE JSON_CONTAINS(type_of_service, '"long_distance_move"', '$')
      `,
      movingAbroad: `
        SELECT *, 'moving_abroad' AS type 
        FROM moving_service 
    ...
    // Modify the WHERE clause addition for moving service queries
    const whereClause = conditions.length
      ? `WHERE ${conditions.join(" AND ")}`
      : "";
    const paginatedQueries = Object.entries(filteredQueries).reduce(
      (acc, [key, query]) => {
        // Check if it's a moving service query
        const isMovingService = [
          "localMove",
          "longDistanceMove",
          "movingAbroad",
        ].includes(key);
        const finalWhereClause = isMovingService
          ? conditions.length
        ...
        return {
          ...acc,
          [key]: `${query} ${finalWhereClause} LIMIT ? OFFSET ?`,
        };
      },
      {}
    );
    ...
    try {
      ...
      const countQueries = Object.entries(filteredQueries).reduce(
        (acc, [key, query]) => {
          ...
          return {
            ...acc,
            [key]: `SELECT COUNT(*) AS count FROM (${query} ${finalWhereClause}) AS subquery`,
          };
        },
        {}
      );
  ...
  async (req, res) => {
    ...
    try {
      ...
      const bids = await new Promise((resolve, reject) => {
        db.query(bidsQuery, [id, type], (err, results) => {
          if (err) {
  ...
  (req, res) => {
    ...
    if (search) {
      query += ` AND (
        ...
        b.status LIKE ? OR
        b.payment_method LIKE ? OR
        b.payment_status LIKE ? OR
        CAST(b.bid_price AS CHAR) LIKE ? OR
        s.company_name LIKE ?
      )`;
    ...
    // Separate base query for counting
    const countQuery = `
    ...
    try {
      const query = `
        SELECT 
          ...
          CONCAT(q.type, '-', q.id, '-', b.id) AS order_id
        FROM bids b
        LEFT JOIN suppliers s ON b.supplier_id = s.id
        INNER JOIN (
          SELECT 
          ...
          FROM company_relocation
          UNION ALL
          SELECT id, 'move_out_cleaning', email_address FROM move_out_cleaning
          UNION ALL
          SELECT id, 'storage', email_address FROM storage
          UNION ALL
          SELECT id, 'heavy_lifting', email_address FROM heavy_lifting
          UNION ALL
          SELECT id, 'carrying_assistance', email_address FROM carrying_assistance
          UNION ALL
          SELECT id, 'junk_removal', email_address FROM junk_removal
          UNION ALL
          SELECT id, 'estate_clearance', email_address FROM estate_clearance
          UNION ALL
          SELECT id, 'evacuation_move', email_address FROM evacuation_move
          UNION ALL
          SELECT id, 'privacy_move', email_address FROM privacy_move
          UNION ALL
          SELECT 
            id,
            'local_move' AS type,
  ...
  async (req, res) => {
    try {
      const countQueries = {
        ...
        quotations: `
          WITH all_counts AS (
            SELECT COUNT(*) as cnt FROM company_relocation
            UNION ALL
            ...
            UNION ALL
            SELECT COUNT(*) FROM moving_service WHERE JSON_CONTAINS(type_of_service, '"local_move"')
            UNION ALL
            SELECT COUNT(*) FROM moving_service WHERE JSON_CONTAINS(type_of_service, '"long_distance_move"')
            UNION ALL
            SELECT COUNT(*) FROM moving_service WHERE JSON_CONTAINS(type_of_service, '"moving_abroad"')
          ),
          awarded_counts AS (
            SELECT COUNT(*) as cnt FROM company_relocation WHERE status = 'awarded'
            UNION ALL
            ...
            UNION ALL
            SELECT COUNT(*) FROM moving_service WHERE status = 'awarded' AND JSON_CONTAINS(type_of_service, '"local_move"')
            UNION ALL
            SELECT COUNT(*) FROM moving_service WHERE status = 'awarded' AND JSON_CONTAINS(type_of_service, '"long_distance_move"')
            UNION ALL
            SELECT COUNT(*) FROM moving_service WHERE status = 'awarded' AND JSON_CONTAINS(type_of_service, '"moving_abroad"')
          ),
          open_counts AS (
            SELECT COUNT(*) as cnt FROM company_relocation WHERE status = 'open'
            UNION ALL
            ...
            UNION ALL
            SELECT COUNT(*) FROM moving_service WHERE status = 'open' AND JSON_CONTAINS(type_of_service, '"local_move"')
            UNION ALL
            SELECT COUNT(*) FROM moving_service WHERE status = 'open' AND JSON_CONTAINS(type_of_service, '"long_distance_move"')
            UNION ALL
            SELECT COUNT(*) FROM moving_service WHERE status = 'open' AND JSON_CONTAINS(type_of_service, '"moving_abroad"')
          )
          SELECT 
            SUM(cnt) AS total,
    ...
    try {
      let query = `
        SELECT 
          ...
          b.payment_method,
          b.payment_status,
          (b.bid_price + (b.bid_price * 0.1)) AS total_amount,
          b.status
        FROM bids b
        INNER JOIN (
          SELECT 
      ...
      if (search) {
        query += ` AND (
          CONCAT(q.type, '-', q.id, '-', b.id) LIKE ? OR
          c.fullname LIKE ? OR
          q.type LIKE ? OR
          q.to_city LIKE ? OR
          s.company_name LIKE ? OR
          b.status LIKE ? OR
          b.payment_status LIKE ?
        )`;
        const searchParam = `%${search}%`;
        queryParams.push(
          searchParam,
      ...
      const offset = (page - 1) * limit;
      const countQuery = `SELECT COUNT(*) AS total FROM (${query}) AS orders`;
...
exports.getOrderById = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        error: "Internal Server Error: Unable to fetch order.",
      });
    }
  },
];
...
exports.getMonthlyBidsTotal = [
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({
        error:
          "Internal Server Error: Unable to fetch monthly completed bids total.",
      });
    }
  },
];
...
exports.editAcceptedBid = [
  ...
  (req, res) => {
    ...
    const checkBidQuery = `
      SELECT 
        ...
        CASE 
          ...
          WHEN b.quotation_type = 'privacy_move' THEN pm.email_address
    ...
    db.query(checkBidQuery, [bid_id], async (checkErr, bids) => {
      ...
      // Determine the table name for the quotation update
          ...
          db.query(updateBidQuery, [finalPrice, bid_id], (updateBidErr) => {
            ...
            db.query(
              ...
              `;
                    ...
                    db.query(
                      getRejectedBidsQuery,
                      [bid.quotation_id, bid.quotation_type, bid_id],
                      async (rejectedErr, rejectedBids) => {
                        if (rejectedErr) {
                          console.error(
                            "Error fetching rejected bids:",
                            rejectedErr
                          );
                        }
                        ...
                        <p>Your bid for ${
                          bid.quotation_type
                        } has been accepted. The bid price is $${bid.amount.toFixed(
                              2
                            )}.</p>
                      ...
                      `,
                          ...
                          await notificationService.createNotification({
                            ...
                            title: "Bid Accepted",
                            message: `Your bid for ${
                              bid.quotation_type
                            } has been accepted. Your bid price is $${bid.amount.toFixed(
                              2
                            )}.`,
                            type: "bid_accepted",
                            referenceId: bid_id,
    ...
    try {
      ...
      await emailService.sendEmail(bid.supplier_email, {
        ...
        html: `
            ...
            <p>We are pleased to inform you that the payment for Bid #${
              bid.bid_id
            }, amounting to $${bid.total_price.toFixed(
          2
      ...
      await notificationService.createNotification({
        ...
        message: `Payment for Bid #${
          bid.bid_id
        } amounting to $${bid.total_price.toFixed(2)} has been disbursed.`,
        type: "payment",
